...
fun validationCodeCorrect(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    ...
    assertEquals("abc", ctx.projectValidated.code)
}

fun validationCodeTrim(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    val ctx = ProjectContext(
        command = command,
        state = InternalState.NONE,
        projectRequest = InternalProject(
...
fun validationCodeEmpty(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    val ctx = ProjectContext(
        command = command,
        state = InternalState.NONE,
        projectRequest = InternalProject(
    ...
    assertEquals("code", error?.field)
    assertContains(error?.message ?: "", "code")
}