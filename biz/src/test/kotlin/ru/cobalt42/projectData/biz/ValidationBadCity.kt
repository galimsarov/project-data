...
fun validationCityCorrect(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    ...
    assertEquals("abc", ctx.projectValidated.city)
}

fun validationCityTrim(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    val ctx = ProjectContext(
        command = command,
        state = InternalState.NONE,
        projectRequest = InternalProject(
...
fun validationCityEmpty(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    val ctx = ProjectContext(
        command = command,
        state = InternalState.NONE,
        projectRequest = InternalProject(
    ...
    assertEquals("city", error?.field)
    assertContains(error?.message ?: "", "city")
}

fun validationCitySymbols(command: InternalCommand, processor: InternalProjectProcessor) = runTest {
    val ctx = ProjectContext(
        command = command,
        state = InternalState.NONE,
        projectRequest = InternalProject(
